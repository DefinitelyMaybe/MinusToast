...
-------------------------------------------------------------------------------
local HandlerBoomerang = HandlerClass.Subclass("HandlerBoomerang")
EternusEngine.EffectsManager:RegisterHandler("HandlerBoomerang", HandlerBoomerang)

HandlerBoomerang.DefaultDamage = 1.0
HandlerBoomerang.DefaultStrikeThreshold = -0.5

-------------------------------------------------------------------------------
function HandlerBoomerang:Constructor(effectHandler, args)
	if args == nil then
		args = {}
	end
...
-------------------------------------------------------------------------------
function HandlerBoomerang:Fire(payload, direction, force)
...
-------------------------------------------------------------------------------
function HandlerBoomerang:Stop()
	self.m_stopped = true
	self:SetTarget(nil)
end

-------------------------------------------------------------------------------
function HandlerBoomerang:OnUpdate(dt)
	...
	if (Eternus.IsServer and targetDistance > 50) then
		...
		--self:SetTarget(--[[newTarget--]] self.m_effectHandler.m_source.object)
	end
	
	--Add a sec of waiting before homing
	self.m_timer = self.m_timer + dt
	
	if self.m_timer >= 0.5 then
		self:SetTarget(--[[newTarget--]] self.m_effectHandler.m_source.object)
		if self.m_target then
			--if targetDistance < 5 then
				--self.m_direction = (self.m_target:NKGetWorldPosition()-pos):NKNormalize()
			--else
				self.m_direction = ((self.m_target:NKGetWorldPosition()+vec3(0,2,0))-pos):NKNormalize()
			--end
		end
	end
	...
	if hit then
		...
		if targetType == EffectsManager.HitType.eCharacter or targetType == EffectsManager.HitType.ePlayer then
...
-------------------------------------------------------------------------------
function HandlerBoomerang:HandlerSerializeConstruction( data )
end

-------------------------------------------------------------------------------
function HandlerBoomerang:HandlerDeserializeConstruction( data )
end

function HandlerBoomerang:HandlerSerialize( stream )
...
function HandlerBoomerang:HandlerDeserialize( stream )
...
--------------------------------------------------------------------------------
function HandlerBoomerang:SetTarget( target )
